...
public Action:OnSayText2(UserMsg:msg_id, Handle:bf, const clients[], numClients, bool:reliable, bool:init)
{
	...
	*/
	...
	{
		Format(cpSender_Name, sizeof(cpSender_Name), "%s", cpSender_Name);
	}
...
public Action:OnSayText(UserMsg:msg_id, Handle:bf, const clients[], numClients, bool:reliable, bool:init)
{
	...
	*/
	...
	if (bProtobuf)
	{
		PbReadString(bf, "text", message, sizeof(message), 1);
	}
	else
	{
	...
	*/
	...
	{
		Format(senderName, sizeof(senderName), "%s", senderName);
	}
...
public OnGameFrame()
{
	for (new i = 0; i < GetArraySize(g_hDPArray); i++)
	{
		...
		else
		{
			...
			Format(message, sizeof(message), "\x01%s\x01%s:\x01 %s", sPrefix, sSenderName, sMessage);
