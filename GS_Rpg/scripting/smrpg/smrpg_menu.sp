...
public TopMenu_HandleUpgrades(Handle:topmenu, TopMenuAction:action, TopMenuObject:object_id, param, String:buffer[], maxlength)
{
	switch(action)
	{
		case TopMenuAction_DisplayOption:
		{
			...
			{
				Format(buffer, maxlength, "%s Lvl %d/%d [%T: %d]%s", sTranslatedName, iCurrentLevel, upgrade[UPGR_maxLevel], "Cost", param, GetUpgradeCost(upgrade[UPGR_index], iCurrentLevel+1), sTeamlock);
			}
		}
		...
		case TopMenuAction_SelectOption:
		{
			...
			else
			{
				...
				{
					CPrintToChat(param,"[{Lightblue}GameSites{default}] %t", "Upgrade bought", sTranslatedName, iItemLevel+1);
