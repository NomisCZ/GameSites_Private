...
Stats_PlayerNewLevel(client, iLevelIncrease)
{
	...
	if(GetConVarBool(g_hCVAnnounceNewLvl))
		CPrintToChatAll("[{Lightblue}GameSites{default}] %t", "Client level changed", client, GetClientLevel(client));
...
Stats_PlayerDamage(attacker, victim, Float:fDamage, const String:sWeapon[])
{
	...
	SMRPG_AddClientExperience(attacker, iExp, ExperienceReason_PlayerHurt, false, victim);
}
...
{
	if(sendto == -1)
		CPrintToChatAll("[{Lightblue}GameSites{default}] %t", "rpgrank", client, GetClientLevel(client), GetClientRank(client), GetRankCount(), GetClientExperience(client), Stats_LvlToExp(GetClientLevel(client)), GetClientCredits(client));
	else
		CPrintToChat(sendto,"[{Lightblue}GameSites{default}] %t", "rpgrank", client, GetClientLevel(client), GetClientRank(client), GetRankCount(), GetClientExperience(client), Stats_LvlToExp(GetClientLevel(client)), GetClientCredits(client));
}
