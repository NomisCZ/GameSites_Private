...
{
	name = "SM:RPG Upgrade > Spy",
	author = "ESK0",
	description = "sSpy upgrade for SM:RPG. Stealth enemy skin.",
...
public OnLibraryAdded(const String:name[])
{
	...
	{
		SMRPG_RegisterUpgradeType("Spy", UPGRADE_SHORTNAME, "Does something.", 10, true, 5, 15, 10, _, SMRPG_BuySell, SMRPG_ActiveQuery);
		SMRPG_SetUpgradeTranslationCallback(UPGRADE_SHORTNAME, SMRPG_TranslateUpgrade);
		g_hCChance = SMRPG_CreateUpgradeConVar(UPGRADE_SHORTNAME, "smrpg_spy_chance", "5", "Chance");
	}
}
...
public Event_OnPlayerHurt(Handle event, const char[] name, bool dontBroadcast)
{
	int victim = GetClientOfUserId(GetEventInt(event, "userid"));
	int attacker = GetClientOfUserId(GetEventInt(event, "attacker"));
	if(IsValidClient(attacker, true) && IsValidClient(victim, true) && GetClientTeam(attacker) != GetClientTeam(victim))
	{
		...
		if(iLevel > 0)
		{
			if(GetChance(iLevel))
			{
				char m_VictimModel[PLATFORM_MAX_PATH];
				GetEntPropString(victim, Prop_Data, "m_ModelName", m_VictimModel, sizeof(m_VictimModel));
				SetEntityModel(attacker, m_VictimModel);
				PrintHintText(attacker, "<font color='#FF0000' size='30'>!! SPY AKTIVOVANÝ !! </font>");
			}
		}
	}
}
